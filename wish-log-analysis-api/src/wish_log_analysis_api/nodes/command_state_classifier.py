...
2. Otherwise, check the command output from `stdout` and `stderr`. Then choose the most appropriate error code from the following:
   - TIMEOUT: When command execution times out or when the output indicates a timeout occurred. Look for indicators such as:
...
Note that OTHERS should only be used when the error does not fit any of the above categories and there are no success indicators in the output.
