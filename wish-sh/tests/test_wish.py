...
class TestWishState:
    def test_enum_values(self):
        """Test that WishState enum has the expected values."""
        assert WishState.CREATED == "created"
        assert WishState.DOING == "doing"
        assert WishState.DONE == "done"
        assert WishState.CANCELLED == "cancelled"
...
class TestWish:
    def test_initialization(self):
        ...
        with patch("uuid.uuid4") as mock_uuid:
            ...
            assert wish.state == WishState.CREATED
...
class TestWishCLI:
    ...
    @patch.object(WishManager, "format_wish_list_item")
    def test_handle_wishlist_select_wish(self, mock_format, mock_handle_details, mock_load_wishes, mock_input, mock_print):
    ...
    @patch.object(WishManager, "execute_command")
    @patch.object(WishManager, "save_wish")
    def test_execute_wish_multiple_commands_select_one(self, mock_save, mock_execute, mock_generate, mock_input, mock_print):
